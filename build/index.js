...
const rentals = [{ studentId: 23, bookIds: [4], date: new Date(Date.now()) },
...
const Map_books = new Map(books.map(livro => [livro.id, livro]));
//Transformandoum array de livros em um array de pares chave-valor, onde cada par é [id, livro].
const Map_student = new Map(students.map(students => [students.id, students]));
...
function getTotalBooksByAuthor(books, author) {
...
function rentBooks(studentId, bookIds) {
    //primeiro verificande se o aluno existe
    function validatingData(studentId, bookIds) {
    ...
    //atualizando dados
    function updateData(studentId, bookIds) {
        for (const bookId of bookIds) {
            const book = books.find(book => book.id === bookId);
            book.quantity--;
            if (book.quantity === 0) {
