...
const Map_livros = new Map(livro.map(livro => [livro.id, livro]));
//Transformandoum array de livros em um array de pares chave-valor, onde cada par é [id, livro].
const Map_estudante = new Map(estudante.map(estudante => [estudante.id, estudante]));
...
function getTotalBooksByAuthor(livros, autor) {
